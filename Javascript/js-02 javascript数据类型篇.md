...
1、在使用var声明变量时，没有赋值，就是undefined；
2、函数没有明确返回值，如果接受了，结果是undefined；
...
注意：null和undefined值没有toString()这个方法，需要使用String()来转换；
...
（6）设置数组：数组名[索引] = 值
（7）获取数据：数组名[索引]；
面试题：var ary = []，ary[0]=1，ary[5]=5，数组的长度是6，没有声明的为undefined；
...
3、使用new操作符执行了哪些过程
（1）创建一个空对象
（2）将这个空对象的__proto__（隐式原型）指向函数的原型；
...
m & (1 === 1)   //false 偶数
```
6、用'~'判断项是否存在，~的特性是：操作数的负值-1；~12为-13；
```js
let firstname = 'ma'
...
5、两个+会是NaN：  'a'+ + 'b'   aNaN 后面这一个b转换为NaN
上题相当于 'a'+(+'b')
