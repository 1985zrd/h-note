...
function createREADME(dirname, name){
  let curDir = fs.readdirSync(resolve('../src/', name, dirname))
  curDir = curDir.filter(file => file.search(/(?<!README)\.md$/) !== -1)
  // 将空格统一替换成20%, 并作为地址
  ...
  fs.writeFileSync(resolve('../src/', name, dirname, 'README.md'), str)
}

module.exports =  function (name) {
  // 获取文件夹并创建README.md文件
  const webArr = fs.readdirSync(resolve('../src/', name))
